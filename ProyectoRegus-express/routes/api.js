...
//contar Sabores
router.get('/Chicle', async(req,res) => {
    await Cerebro.find({flavor: 'Chicle'}).count().exec((error,sabores) => {
        if(!error)
        {
            console.log(sabores);
...
router.get('/Melon', async(req,res) => {
    await Cerebro.find({flavor: 'Melón'}).count().exec((error,sabores) => {
        if(!error)
        {
            console.log(sabores);
            res.status(200).json(sabores);
...
router.get('/Oreo', async(req,res) => {
    await Cerebro.find({flavor: 'Galleta Oreo'}).count().exec((error,sabores) => {
        if(!error)
        {
            console.log(sabores);
            res.status(200).json(sabores);
